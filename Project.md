...
When prefill-decode disaggregation is enabled (`allow_pdd`), the engine slots are split into two disjoint pools — a prefill pool and a decode pool — with the partition point set by `prefill_engine_fraction` (a fixed fraction for now; dynamic repartitioning is left as future work, since fixed partitions tend to be predictably suboptimal). A request is prefilled on the prefill pool; on completion its KV cache is moved to the decode pool as a real, bandwidth-contended transfer of the prompt's KV bytes over the link between the two engines' memories — the same physics as every other move, so it occupies the bandwidth of both the prefill engine's memory it reads from and the decode engine's memory it writes into, contends with concurrent work sharing either end, and surfaces in the per-memory bandwidth reports. Decode begins only after that transfer completes. Parallel handoffs are spread across the decode engines (as their decode batches will be), so distinct sequences do not falsely contend a single memory. Prefill and decode batch independently, while max concurrency counts a sequence as in flight from prefill dispatch through decode completion.
...
**Completion (offload/migration).** When a turn completes, its full context KV (prompt + generated tokens) is moved from the serving device into a floating memory as a modeled, arbiter-accounted transfer (a `kv_transfer` decision), freeing the device's first-tier memory for the next batch. To keep offloaded KV from piling onto a single node memory — which would turn that one node's bandwidth into an unrealistic bottleneck — each sequence is assigned a *home* floating memory chosen by hashing its `(workload_id, turn_index)` key. The whole sequence always lands on a single memory (it is never split across nodes, which would be impractical to implement in a real system); this spreads the aggregate offload load evenly across the node memories. If the home node is full, the next nodes are probed (wrapping around) so capacity constraints are still honoured.
...
As the run progresses it prints a log of major simulation milestones:
1. Arrival of sequences to the queue (including unique sequence ID and the total done and remaining in test suite, input and output token count in sequence)
2. Issue of sequences (including the batch ID and contents, and the engine group executing it (identified using the first device + number notation))
3. Completion of sequences - including reporting time in queue, TTFT, and TPS
Each event will print the simulation time, wall time, and the information above

The option `--quiet` suppresses these updates and emits a simple progress of sequences done vs. remaining
...
- Per-device utilization (compute and bandwidth) and memory occupancy over time. Memory occupancy is the reserved per-device footprint (weights + KV) of the jobs active at each instant, as sized by the parallelism planner -- a reservation estimate, not a byte-accurate residency trace. This occupancy can never exceed a device's memory: a run that would oversubscribe any device aborts with an out-of-memory error (see "Memory is a hard constraint" above) instead of producing a report. Floating (node) memories hold no device-reserved footprint; their occupancy is instead reconstructed from the event log -- the offloaded KV resident there (from `kv_transfer`/`kv_eviction`) plus any homed model shard staged into them (the once-per-`(model, node)` NVM->RAM `weight_transfer`, which carries the model's weights *and* the experts it later streams), each held from the staging completion to the end of the run.
